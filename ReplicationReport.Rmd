...
```{r}
sem <- function(x) {sd(x, na.rm=TRUE) / sqrt(sum(!is.na((x))))} # formula for sem

d.plot <- d.results %>%
  ungroup() %>%
  group_by(trial_id,alignment,congruency) %>%
  summarize(avg = mean(d_prime), error = sem(d_prime))
d.plot %>%
  ggplot(aes(x= alignment, y = avg)) +
  facet_grid(.~ trial_id) +
  geom_point(aes(col=congruency)) +
  geom_errorbar(aes(ymin=avg-error, ymax=avg+error), width=.1) +
  geom_line(aes(group = congruency,col=congruency)) +

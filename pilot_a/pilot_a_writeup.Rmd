...
In their 2016 Psychological Science paper, Zhao, Bülthoff and Bülthoff showed that nonface objects, specifically line patterns with salient Gestalt information, can elicit holistic processing, even in the absence of expertise. In this paper, holistic processing was measured by composite tasks, where the dependent variable was response sensitivity (d') and a significant interaction between congruency and alignment in the line composite task (driven by a much larher congruency effectin the aligned conditions than in the misaligned conditions) was taken as evidence of holistic processing for the line pattern stimuli. We aim to replicate Experiment 1a of this paper, where participants are presented with a line composite task and a face composite task in a single session. In that experiment, the authors found evidence for holistic processing of both stimuli types (line and face).

##Methods

###Power Analysis

The effect we are attempting to replicate is the interaction between congruency and alignment in the line pattern task (ηp2 = .52). This original experiment had 22 participants, giving 99.9% a posteriori power with an alpha level of 0.05. A post-hoc power analysis given this effect size and alpha level determines that we would need 8 participants for 80% power, 8 participants for 90% power, and 10 participants for 95% power. Power calculations were run in G*power with the 'Options' changed so that the effect size specification was set 'as in SPSS'. 

###Planned Sample

Given the large effect size and relatively small original sample size, we planned to use the same sample size as the original authors (N = 22). However, for reasons of cost, we then reduced from original sample size of 22 to sample size of 15. 
We plan to use an overall > 55% accuracy cutoff for excluding participants. 
...
For the replication we obtained the line patterns used from the authors, so those stimuli were identical to the original study. However, due to copyright issues, we were not able to obtain the original face stimuli. Instead, we used the face stimuli available from the [Rossion lab](face-categorization-lab.webnode.com/resources/) (Rossion, 2013). However, there were only enough stimuli for 10 pairs of face pairs, so each pair was repeated to create the original number of stimuli. 
...
* We are using a sample size of 15 instead of 22. This should still provide 99% power.
* The face stimuli used are different. The stimuli we used instead have been shown to consistently yield face composite effects so this is not anticipated to make a difference. 
* We do not have practice trials for the face composite task due to the lower number of available face stimuli.
* We are not counterbalancing the tasks.
* We are conducting the experiment on Mechanical Turk, not in the lab. Note, however, that the composite face effect has been consistently shown with these presentation times using samples from Mechanical Turk (Susilo, Rezlescu & Duchaine 2013).
...
d$alignment <- as.factor(d$alignment)
...
t.line.aligned$p.value <- t.line.aligned$p.value/num_tests_line
...
t.line.misaligned$p.value <- t.line.misaligned$p.value/num_tests_line
...
t.face.aligned$p.value <- t.face.aligned$p.value/num_tests_face
...
t.face.misaligned$p.value <- t.face.misaligned$p.value/num_tests_face
